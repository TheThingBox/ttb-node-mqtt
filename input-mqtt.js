...
module.exports = function(RED) {
    ...
    var brokers = {
        ...
        get: function(n) {
            ...
            if (index == -1) {
                ...
                b.options.username = b.username;
                b.options.password = b.password;
    ...
    function compareBrokerConfig(a, b) {
        if (a && b &&
            ...
            ((!a.credentials && !b.credentials) || (a.credentials && b.credentials && a.credentials.user == b.credentials.user && a.credentials.password == b.credentials.password))
        ) {
            return true;
        }
    ...
    function mainInput(config) {
        ...
        if (canHaveEarlyBroker(node.param)) {
            ...
            if (node.param.user != "" && node.param.password != "") {
                credentials = {
                    user: getProperty(node.param, msg, "user", "userType", { value: "user", type: "setUser" }, "str", "none", true, node.warn),
                    password: getProperty(node.param, msg, "password", "passwordType", { value: "password", type: "setPassword" }, "str", "none", true, node.warn)
                }
            }
            if (node.connect({
    ...
    RED.nodes.registerType("input-mqtt", mainInput);
    ...
    RED.nodes.registerType("output-mqtt", mainOutput);
}
